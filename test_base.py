...
def valid():
    ...
    seq_len = 64
    model = Unet(dim=64, dim_mults=(1, 2), channels=1, self_condition=False)
    ...
    if PC:
        ...
        diffusion.load_state_dict(torch.load('../result/simulation_pc/epoch_model_{}.pkl'.format(epoch)))
    else:
        raw_data = None
        diffusion = GaussianDiffusion(model=model, device=device, timesteps=400, sampling_timesteps=200, beta_schedule='linear', objective='pred_noise')
